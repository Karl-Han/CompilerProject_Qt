...
// maximum line size and word size
#define LINESIZE 121
#define WORDSIZE 20
...
// instructions
// all of them involve with register
//  so the problem is what is the sturcture of the machine like
typedef enum
{
  // ins: iop r, d(s)
  /* RR instructions */
...
int doCommand(void)
{
  ...
  // ~~what if it is `s 10`, step 10 ins~~
  // this do while deal with s
  do
  {
  ...
  if (stepcnt > 0)
  {
    ...
    if (cmd == 'g')
    {
      ...
      while (stepResult == srOKAY)
      {
        iloc = reg[PC_REG];
        if (traceflag)
          // write the PC and the respective instruction
          writeInstruction(iloc);
        stepResult = stepTM();
        stepcnt++;
      }
